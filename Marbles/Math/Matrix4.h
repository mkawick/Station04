...
public:
	...
	friend Vector3			operator *			(const Vector3& vect, const Matrix4& mat);
	friend Matrix4			operator /			(const Matrix4& mat, const float f);
	...
	void					SetRow				(int row, const Vector3& vect);
	
	Vector3					GetCol				(const int col) const;
	void					SetCol				(int col, const Vector3& vect);
	
	Vector3					GetXbasis			() const;
	Vector3					GetYbasis			() const;
	Vector3					GetZbasis			() const;
	const Matrix4&			SetXbasis			(const Vector3& vect);
	const Matrix4&			SetYbasis			(const Vector3& vect);
	const Matrix4&			SetZbasis			(const Vector3& vect);
	
	Matrix4&				SetIdentity			();
	Matrix4&				SetTranslation		(float x, float y, float z);
	Matrix4&				SetTranslation		(const Vector3& vect);
	...
	Vector3					GetTranslation		() const; // retrieve the translation vector only
	Vector3					GetScale			() const; // retrieve the scale vector only
...
public:
	...
	static	Matrix4			Rotation	(const Vector3& vect); //rotation angle
	...
	static	Matrix4			Translation  (const Vector3& vect);// considered to be a distance vector (normalized will probably not work well)
	...
	static	Matrix4			Scale	 (const Vector3& vect); // scale_factor
