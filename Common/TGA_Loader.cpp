...
int		TGA_Loader::Load( const char* filename )
{
	using namespace std;
	ifstream		fIn;
...
int TGA_Loader::ReadHeader() // Examine the header and populate our class attributes
{
...
int TGA_Loader::LoadRawData() // Load uncompressed image data
{
	short iOffset;
...
int TGA_Loader::LoadTgaRLEData() // Load RLE compressed image data
{
...
int TGA_Loader::LoadTgaPalette() // Load a 256 color palette
{
	unsigned char bTemp;
...
void TGA_Loader::BGRtoRGB() // Convert BGR to RGB (or back again)
{
...
void TGA_Loader::FlipImg() // Flips the image vertically (Why store images upside down?)
{
	unsigned char bTemp;
...
int TGA_Loader::GetBPP()
{
	return iBPP;
}


int TGA_Loader::GetWidth()
{
	return iWidth;
}


int TGA_Loader::GetHeight()
{
	return iHeight;
}


unsigned char* TGA_Loader::GetImg()
{
	return pImage;
}


unsigned char* TGA_Loader::GetPalette()
{
	return pPalette;
}